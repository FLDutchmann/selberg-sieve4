...
theorem finMulAntidiagonal_empty_of_ne_one (hn : n ≠ 1) :
...
theorem ne_zero_of_mem_finMulAntidiagonal {d n : ℕ} {f : Fin d → ℕ} (hf : f ∈ finMulAntidiagonal d n) (i : Fin d) :
    f i ≠ 0 :=
  ne_zero_of_dvd_ne_zero (mem_finMulAntidiagonal.mp hf).2 (dvd_of_mem_finMulAntidiagonal hf i)

theorem prod_eq_of_mem_finMulAntidiagonal {d n : ℕ} {f : Fin d → ℕ} (hf : f ∈ finMulAntidiagonal d n):
    ∏ i, f i = n :=
...
private theorem primeFactorsPiBij_inj (d n : ℕ)
    ...
    (hg : g ∈ pi n.primeFactors fun _ => univ) : Nat.primeFactorsPiBij d n f hf = Nat.primeFactorsPiBij d n g hg → f = g := by
  contrapose!
...
--   rw [card_finMulAntidiagonal hd, card_fin]
...
private theorem f_img {n : ℕ} (hn : Squarefree n) : ∀ (a : Fin 3 → ℕ) (ha : a ∈ finMulAntidiagonal 3 n),
...
theorem card_lcm_eq {n : ℕ} (hn : Squarefree n) :
    Finset.card ((n.divisors ×ˢ n.divisors).filter fun ⟨x, y⟩ => x.lcm y = n) =
      3 ^ ω n := by
  rw [← card_finMulAntidiagonal hn, eq_comm]
