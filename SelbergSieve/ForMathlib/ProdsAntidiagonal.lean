...
def piMulAntidiagonal (s : Finset ι) (n : α)  : Set (ι → α) := 
  { f : ι → α | ∏ i in s, f i = n ∧ ∀ i, i ∉ s → f i = 1 }

@[simp]
theorem mem_piMulAntidiagonal {s : Finset ι} {n : α} {f : ι → α} : 
    f ∈ piMulAntidiagonal s n ↔ ∏ i in s, f i = n ∧ ∀ i, i ∉ s → f i = 1 := by
  simp [piMulAntidiagonal]

theorem prod_eq_of_mem_piMulAntidiagonal {s : Finset ι} {n : α} {f : ι → α} 
    (hf : f ∈ piMulAntidiagonal s n) :
    ∏ i in s, f i = n := by
  exact (mem_piMulAntidiagonal.mp hf).1

theorem dvd_of_mem_piMulAntidiagonal {s : Finset ι} {n : α} {f : ι → α} 
    (hf : f ∈ piMulAntidiagonal s n) (i : ι) : 
    f i ∣ n := by
...
open Classical in
noncomputable def piMulAntidiagonal (s : Finset ι) (n : α)  : Finset (ι → α) := 
  if h : (Set.piMulAntidiagonal s n).Finite then
    h.toFinset
  else ∅

theorem mem_piMulAntidiagonal_of_finite {s : Finset ι} {n : α} {f : ι → α}
    (h : (Set.piMulAntidiagonal s n).Finite) :
    f ∈ piMulAntidiagonal s n ↔ ∏ i in s, f i = n ∧ (∀ i, i ∉ s → f i = 1) := by
  unfold piMulAntidiagonal
  rw [dif_pos h, Set.Finite.mem_toFinset, Set.mem_piMulAntidiagonal]

theorem piMulAntidiagonal_of_infinite {s : Finset ι} {n : α}
    (h : (Set.piMulAntidiagonal s n).Infinite) :
    piMulAntidiagonal s n = ∅ := by
  unfold piMulAntidiagonal
  rw [dif_neg h]

theorem mem_piMulAntidiagonal {s : Finset ι} {n : α} {f : ι → α} : 
    f ∈ piMulAntidiagonal s n ↔ ∏ i in s, f i = n ∧ (∀ i, i ∉ s → f i = 1) 
      ∧ Set.Finite (Set.piMulAntidiagonal s n) := by
  by_cases h : Set.Finite (Set.piMulAntidiagonal s n)
  · have test := mem_piMulAntidiagonal_of_finite h (f:=f)
    tauto
  · rw [piMulAntidiagonal_of_infinite h]; tauto
...
theorem mem_piMulAntidiagonal_of_finite (n : α) (s : Finset ι) (f : ι → α) (hn : Set.Finite (Set.Iic n)) :
    f ∈ piMulAntidiagonal s n ↔ (∏ i in s, f i = n) ∧ (∀ i, i ∉ s → f i = 1) := by
  unfold piMulAntidiagonal
  rw [dif_pos hn]
  rw [mem_filter]
...
theorem mem_piMulAntidiagonal_of_empty (n : α) (s : Finset ι) (f : ι → α) (hs : s = ∅) :
    f ∈ piMulAntidiagonal s n ↔ (n = 1) ∧ (∀ i, f i = 1) := by
  unfold piMulAntidiagonal
...
theorem mem_piMulAntidiagonal (n : α) (s : Finset ι) (f : ι → α) :
    f ∈ piMulAntidiagonal s n ↔ (∏ i in s, f i = n) ∧ (∀ i, i ∉ s → f i = 1) ∧ (s = ∅ ∨ Set.Finite (Set.Iic n)) := by
  --unfold piMulAntidiagonal
  by_cases hn : Set.Finite (Set.Iic n)
  · have := mem_piMulAntidiagonal_of_finite n s f hn
    tauto
  by_cases hs : s = ∅
  · have := mem_piMulAntidiagonal_of_empty n s f hs
    rw [this]
    simp [hs]
    exact fun _ => eq_comm
  simp [hn, hs]
...
theorem mem_piMulAntidiagonal (n : Associates ℕ) (s : Finset ι) (f : ι → Associates ℕ) :
    f ∈ piMulAntidiagonal s n ↔ (∏ i in s, f i = n) ∧ (∀ i, i ∉ s → f i = 1) ∧ n ≠ 0 := by
  sorry 

theorem mem_piMulAntidiagonal_univ [Fintype ι] (n : Associates ℕ) (f : ι → Associates ℕ) :
    f ∈ piMulAntidiagonal univ n ↔ (∏ i, f i = n) ∧ n ≠ 0 := by
  sorry
