...
-- TODO: rename nu to ν so as to not clash with arithmetic_function
-- TODO: make nu an arithmetic_function so we can re-use is_multiplicative  
...
theorem hP_ne_zero : s.P ≠ 0 :=
  Squarefree.ne_zero s.hP

theorem sqfree_of_dvd_P {d : ℕ} (hd : d ∣ s.P) : Squarefree d :=
  Squarefree.squarefree_of_dvd hd s.hP
theorem sqfree_of_mem_dvd_P {d : ℕ} (hd : d ∈ s.P.divisors) : Squarefree d :=
  by
  simp only [Nat.mem_divisors, Ne.def] at hd 
  exact Squarefree.squarefree_of_dvd hd.left s.hP

theorem nu_pos_of_dvd_P {d : ℕ} (hl : d ∣ s.P) : 0 < ν d := by
  calc
    ...
    _ = ν d := prod_factors_of_mult ν s.nu_mult (Squarefree.squarefree_of_dvd hl s.hP)

theorem nu_ne_zero_of_mem_dvd_P {d : ℕ} (hd : d ∈ s.P.divisors) : ν d ≠ 0 :=
  by
...
@[simp]
def R (d : ℕ) : ℝ :=
  s.multSum d - ν d / d * s.X

theorem multSum_eq_main_err (d : ℕ) : s.multSum d = (ν d) / (d:ℝ) * s.X + s.R d :=
  by
  dsimp [R]
  ring
...
theorem siftedSum_as_delta : s.siftedSum = ∑ d in s.A, s.a d * δ (Nat.gcd s.P d) :=
  by
...
def nu_lt_self_of_dvd_P : ∀ d : ℕ, d ∣ s.P → d ≠ 1 → ν d < d :=
  by
...
-- S = ∑_{l|P, l≤√y} g(l)
-- Facts about g
theorem hg_pos (l : ℕ) (hl : l ∣ s.P) : 0 < s.g l :=
  by
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (k l) -/
theorem omega_eq_conv_rec_g (d : ℕ) (hdP : d ∣ s.P) :
    (d : ℝ) / ν d = ∑ l in s.P.divisors, if l ∣ d then 1 / s.g l else 0 :=
  by
  ...
  calc
    (∑ l in s.P.divisors, if l ∣ d then 1 / s.g l else 0) =
        ∑ l in s.P.divisors, if l ∣ d then ∑ k in l.divisors, (μ <| l / k) * (k / ν k) else 0 :=
      by
      apply sum_congr rfl; intro l hl
      rw [s.rec_g_eq_conv_moebius_omega l (s.sqfree_of_mem_dvd_P hl) (s.nu_ne_zero_of_mem_dvd_P hl)]
    _ =
        ∑ l in s.P.divisors,
          if l ∣ d then ∑ k in s.P.divisors, if k ∣ l then (μ <| l / k) * (k / ν k) else 0
          else 0 :=
      by
      apply sum_congr rfl; intro l hl
    ...
    _ =
        ∑ l in s.P.divisors,
          (∑ k in s.P.divisors, if k ∣ l then (μ <| l / k) * (k / ν k) else 0) *
    ...
    _ = ∑ k in s.P.divisors, ∑ l in s.P.divisors, (if k ∣ l ∧ l ∣ d then (μ <| l / k) * (k / ν k) else 0) :=
      by
      rw [sum_comm]
      apply sum_congr rfl; intro l hl
    ...
    _ = ∑ k in s.P.divisors, ∑ l in s.P.divisors, (if k ∣ l ∧ l ∣ d then (μ l:ℝ) / (μ k:ℝ) else 0) * (k / ν k) :=
      by
      apply sum_congr rfl; intro k hk
      apply sum_congr rfl; intro l hl
    ...
    _ =
        ∑ k in s.P.divisors,
          (∑ l in s.P.divisors, if k ∣ l ∧ l ∣ d then (μ l:ℝ) else 0) / (μ k:ℝ) * (k / ν k) :=
      by
      apply sum_congr rfl; intro k hk
      conv =>
        lhs
    ...
    _ = ∑ k in s.P.divisors, (if k = d then (μ k:ℝ) else 0) / (μ k:ℝ) * (k / ν k) :=
      by
...
theorem conv_g_eq {d : ℕ} (hd : d ∣ s.P) :
    (∑ l in s.P.divisors, if l ∣ d then s.g l else 0) = s.g d * (↑d / ν d) := by
  calc
...
def mainSum (μPlus : ℕ → ℝ) : ℝ :=
  ∑ d in s.P.divisors, μPlus d * ν d / d

def errSum (μPlus : ℕ → ℝ) : ℝ :=
  ∑ d in s.P.divisors, |μPlus d| * |s.R d|

theorem upper_bound_of_upperBoundSieve (μPlus : UpperBoundSieve) :
    s.siftedSum ≤ ∑ d in s.P.divisors, μPlus d * s.multSum d :=
  by
  have hμ : ∀ n, δ n ≤ ∑ d in n.divisors, μPlus d := μPlus.hμPlus
  rw [siftedSum_as_delta]
  calc
    ∑ n in s.A, s.a n * δ (s.P.gcd n) ≤ ∑ n in s.A, s.a n * ∑ d in (s.P.gcd n).divisors, μPlus d :=
      by
      apply Finset.sum_le_sum
      intro n hnA
      exact mul_le_mul_of_nonneg_left (hμ (s.P.gcd n)) (s.ha_nonneg n)
    _ = ∑ n in s.A, ∑ d in (s.P.gcd n).divisors, s.a n * μPlus d :=
      by
      apply sum_congr rfl; intro n hnA
      exact mul_sum
    _ = ∑ n in s.A, ∑ d in s.P.divisors, if d ∣ n then s.a n * μPlus d else 0 :=
      by
      apply sum_congr rfl; intro n hnA
      ...
      · intro d hd
        ...
        exact if_pos this
    _ = ∑ d in s.P.divisors, ∑ n in s.A, if d ∣ n then s.a n * μPlus d else 0 := Finset.sum_comm
    _ = ∑ d in s.P.divisors, ∑ n in s.A, μPlus d * if d ∣ n then s.a n else 0 :=
      by
      apply sum_congr rfl; intro d hdP
      apply sum_congr rfl; intro n hnA
      rw [ite_mul_zero_left]; ring
    _ = ∑ d in s.P.divisors, μPlus d * ∑ n in s.A, if d ∣ n then s.a n else 0 :=
      by
      apply sum_congr rfl; intro d hdP
      rw [eq_comm]
      apply mul_sum
...
theorem upper_bound_main_err (μPlus : UpperBoundSieve) :
    s.siftedSum ≤ s.X * s.mainSum μPlus + s.errSum μPlus :=
  by
  dsimp only [mainSum, errSum]
  have err_bound :
    ∑ d in s.P.divisors, μPlus d * s.R d ≤ ∑ d in s.P.divisors, |μPlus d| * |s.R d| := by
    calc
      ∑ d in s.P.divisors, μPlus d * s.R d ≤ |∑ d in s.P.divisors, μPlus d * s.R d| :=
        le_abs.mpr (Or.intro_left _ le_rfl)
      _ ≤ ∑ d in s.P.divisors, |μPlus d * s.R d| :=
        (abs_sum_le_sum_abs (fun d => μPlus d * s.R d) s.P.divisors)
      _ = ∑ d in s.P.divisors, |μPlus d| * |s.R d| :=
        by
        apply sum_congr rfl; intro d hdP
  ...
  calc
    ...
    _ = ∑ d in s.P.divisors, μPlus d * (ν d / d * s.X + s.R d) :=
      by
      apply sum_congr rfl; intro d hdP
      rw [s.multSum_eq_main_err]
    _ = ∑ d in s.P.divisors, (μPlus d * ν d / d * s.X + μPlus d * s.R d) := by
      apply sum_congr rfl; intro d hdP; ring
    _ = ∑ d in s.P.divisors, μPlus d * ν d / d * s.X + ∑ d in s.P.divisors, μPlus d * s.R d :=
      sum_add_distrib
    _ = s.X * ∑ d in s.P.divisors, μPlus d * ν d / ↑d + ∑ d in s.P.divisors, μPlus d * s.R d :=
      by rw [← sum_mul]; rw [mul_comm]
    _ ≤
        s.X * ∑ d in s.P.divisors, μPlus d * ν d / ↑d +
          ∑ d in s.P.divisors, |μPlus d| * |s.R d| :=
      by linarith
  ...
  calc
    ...
    _ = ∑ d in s.P.divisors, ∑ d1 in s.P.divisors, ∑ d2 in s.P.divisors, 
      ...
      · intro d2 hd2
        ...
        rw [if_neg this]
      · exact fun _ _ => rfl
    _ = ∑ d1 in s.P.divisors, ∑ d2 in s.P.divisors, ∑ d in s.P.divisors,  if d = d1.lcm d2 then w d1 * w d2 * ν d / ↑d else 0 := 
    ...
    _ =
      ...
      rw [← sum_intro s.P.divisors True (w d1 * w d2 * ν (d1.lcm d2) / ↑(d1.lcm d2))]
...
theorem lambda_sq_mainSum_eq_diag_quad_form  (w : ℕ → ℝ) :
    s.mainSum (lambdaSquaredOfWeights w) =
      ∑ l in s.P.divisors,
        1 / s.g l * (∑ d in s.P.divisors, if l ∣ d then ν d / d * w d else 0) ^ 2 :=
  by
  ...
  calc
    ...
    _ =
        ∑ d1 in s.P.divisors, ∑ d2 in s.P.divisors,
          (∑ l in s.P.divisors, if l ∣ d1.gcd d2 then 1 / s.g l else 0) * (ν d1 / ↑d1 * w d1) *
            (ν d2 / ↑d2 * w d2) :=
      by
      apply sum_congr rfl; intro d1 hd1; apply sum_congr rfl; intro d2 hd2
    ...
    _ =
      ...
      suffices :
        (∑ d1 in s.P.divisors, ∑ d2 in s.P.divisors,
            if l ∣ d1 ∧ l ∣ d2 then ν d1 / ↑d1 * w d1 * (ν d2 / ↑d2 * w d2) else 0) =
          (∑ d in s.P.divisors, if l ∣ d then ν d / ↑d * w d else 0) ^ 2
      rw [this, cast_one]
      rw [sq]
      rw [mul_sum]; apply sum_congr rfl; intro d1 hd1
